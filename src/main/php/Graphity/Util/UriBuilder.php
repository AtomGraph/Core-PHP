...
class UriBuilder {
    ...
    public static function fromUri($uri) {
        ...
        if(array_key_exists('fragment', $components)) {
            $builder->fragment($components['fragment']);
        }
    ...
    public static function fromPath($path) {
        ...
        if(array_key_exists('path', $components)) {
            $builder->listOfSegments = explode("/", $components['path']);
        }
    ...
    protected function buildWithTemplate($template) {
        ...
        if(!empty($this->port)) {
            if($this->scheme !== null && (($this->scheme == "http" && intval($this->port) !== 80) || ($this->scheme == "https" && intval($this->port) !== 443))) {
                $uri .= ":" . $this->port;
            }
        }
