...
class Router
{
    ...
    public function buildURI($resource, array $params = array())
    {
        ...
        if(! array_key_exists($resource, $this->routes)) {
            throw new \Graphity\Exception("Could not find route for resource: '{$resource}'.");
        }
        ...
        foreach($uri as $idx => $segment) {
            ...
            if(! array_key_exists($segment, $params)) {
                throw new \Graphity\Exception("Missing parameter '{$segment}' when generating route for resource: '{$resource}'.");
            }
        ...
        if(preg_match($this->routes[$resource]['matchPath'], $uri) === 0) {
            throw new \Graphity\Exception("Could not generate valid URI for resource '{$resource}': '{$uri}'.");
        }
